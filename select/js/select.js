$.fn.select = function (o) {
    ...
    getPlaceholder()
    let sWrapper = $(`<div class="select-wrapper select-wrapper-${id}" data-select="${id}"></div>`);
    let sSelected = $(`<div class="select-wrapper__selected"></div>`);
    let sSelectedText = $(`<span class="select-wrapper__selected-text ${!showPlaceholder ? 'select-wrapper__placeholder' : ''}">${placeholder ? placeholder : 'select option'}</span>`);
    ...
    let sSearch = $(`<input type="text" class="select-wrapper__search" placeholder="search">`);
    let sArrow = $(`<span class="select-wrapper__arrow"></span>`);
    let sOptions = $(`<div class="select-wrapper__options"></div>`);
    options.each(function () {
        let sOption = $(`<div class="select-wrapper__option" data-value="${$(this).attr('value')}">${$(this).text()}</div>`);
        sOptions.append(sOption);
    })
    let sOptionsNoResultFound = $(`<div class="select-wrapper__option select-wrapper__no-result hide">No result found</div>`);
    ...
    sSelected.bind('click', function (e) {
        $(this).closest('.select-wrapper').toggleClass('select-wrapper__opened').find('.select-wrapper__dropdown').slideToggle(200);
    })
    sOptions.find('.select-wrapper__option').bind('click', function () {
        ...
        selectWrapperEle.find('.select-wrapper__selected-text').text(text).removeClass('select-wrapper__placeholder');
    ...
    sSearch.bind('keyup', function () {
        ...
        let selectWrapperEle = $(this).closest('.select-wrapper')
        if (value == '') {
            selectWrapperEle.find('.select-wrapper__option').show();
            return;
        }
        let options = selectWrapperEle.find('.select-wrapper__option:not(.select-wrapper__no-result)');
        options.each(function () {
        ...
        if (selectWrapperEle.find('.select-wrapper__option:not(.select-wrapper__no-result):visible').length == 0) {
